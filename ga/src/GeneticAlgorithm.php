...
class GeneticAlgorithm
{
    ...
    public function optimiseUntil(SuccessCriteriaInterface $successCriteria, $maxIterations = 50000)
    {
        ...
        do {
            $count++;
            $this->runIteration();
            //echo "Progress " . ($count / $maxCount * 100) . " %\r";
    ...
    protected function runIteration()
    {
        ...
        //echo $cullCount . PHP_EOL . $crossoverCount . PHP_EOL . $mutateCount . PHP_EOL;die;
        $this->cull($cullCount);
        ...
        $stats   = $this->getStats();
        $content = $stats['populationCount'] . ',' . $stats['bestFitness'] . ',' . $stats['averageFitness'] . PHP_EOL;
        file_put_contents('statistics/stats' . $GLOBALS['file'] . '.csv', $content, FILE_APPEND);
        //$fittest = $this->getFittest()->getValue();
        //file_put_contents('best' . $GLOBALS['file'] . '.csv', implode(',', $fittest) . PHP_EOL, FILE_APPEND);
    }
